...
const Sidebar = () => {
    ...
    // Filter users (exclude logged in user)
    ...
    return (
        <div className={`bg-[#120e26]/80 backdrop-blur-xl h-full min-h-0 flex flex-col border-r border-white/10 text-white overflow-hidden ${selectedUser ? "max-md:hidden" : ''}`}>
            {/* Header Section - flex-shrink-0 */}
            <div className="flex-shrink-0 p-5 border-b border-white/5 flex flex-col gap-4">
                ...
                {/* Search Bar */}
                <div className='bg-white/[0.06] focus-within:bg-white/[0.1] focus-within:ring-2 focus-within:ring-violet-500/50 border border-white/10 rounded-2xl flex items-center gap-2 py-2.5 px-3.5 transition-all duration-200'>
            ...
            {/* Users List Section - flex-1 min-h-0 overflow-y-auto */}
            <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-1.5 scrollbar-thin">
                ...
                {isUsersLoading ? (
                    <div className="flex flex-col gap-2 pt-4">
                        {[1, 2, 3, 4].map((n) => (
                            <div key={n} className="flex items-center gap-3 p-3 rounded-2xl bg-white/[0.03] animate-pulse">
                    ...
                    filteredUsers.map((user) => {
                        ...
                        return (
                            <div
                                ...
                                className={`relative flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all duration-200 border ${
                                    isSelected
                            ...
                            >
                                {/* Avatar with Glowing Online Status Ring */}
                                <div className="relative flex-shrink-0">
                                    <img
                                        src={user?.profilePic || assets.avatar_icon}
                                        alt={user.fullName}
                                        className={`w-11 h-11 rounded-full object-cover ring-2 ${
                                            isSelected ? 'ring-violet-400' : 'ring-white/10'
                                        }`}
                                    />
                                    {isOnline && (
                                        <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 border-2 border-[#120e26] rounded-full shadow-[0_0_8px_#10b981]"></span>
                                    )}
                                </div>
                                ...
                                {unseenCount > 0 && (
                                    <div className='flex-shrink-0 min-w-[20px] h-5 px-1.5 flex justify-center items-center rounded-full bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-[11px] font-bold shadow-md shadow-violet-500/40 animate-pulse'>
                                        {unseenCount}
                                    </div>
                                )}
                            </div>
